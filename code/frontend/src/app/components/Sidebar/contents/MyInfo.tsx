...
const Graph = () => {
  ...
  // 랜덤 데이터 생성
  const generateRandomDataRed = () => {
    return Array.from({ length: timesLength }, () =>
      Math.floor(Math.random() * 101)
    );
  };

  const generateRandomDataBlue = () => {
    return Array.from({ length: timesLength }, () =>
      Math.floor(Math.random() * 101)
    );
  };

  const randomDataRed = generateRandomDataRed();
  const randomDataBlue = generateRandomDataBlue();
            ...
            <>
              ...
              {i < a - 1 && (
                <line
                  key={`line-${i}`}
                  x1={b * i}
                  y1={100 - randomDataRed[i]}
                  x2={b * (i + 1)}
                  y2={100 - randomDataRed[i + 1]}
                  stroke="red"
                  strokeWidth="0.8"
                />
              )}
              <circle
                cx={b * i}
                cy={100 - randomDataRed[i]} // 랜덤 데이터 값을 y 좌표로 사용
                r="1.1"
                fill="red"
              />
              {i < a - 1 && (
                <line
                  key={`line-${i}`}
                  x1={b * i}
                  y1={100 - randomDataBlue[i]}
                  x2={b * (i + 1)}
                  y2={100 - randomDataBlue[i + 1]}
                  stroke="blue"
                  strokeWidth="0.8"
                />
              )}
              <circle
                cx={b * i}
                cy={100 - randomDataBlue[i]} // 랜덤 데이터 값을 y 좌표로 사용
                r="1.1"
                fill="blue"
              />
            </>
